...
public class LinkedList<T> implements Iterable<T> {
    ...
    public <R> LinkedList<R> map(Function<T, R> mapper) {
        ...
        LinkedList<R> treateadList = new LinkedList<>();
        for (T elements : this) {
            treateadList.add(mapper.apply(elements));
        }
    ...
    public LinkedList<T> filter(Predicate<T> predicate) {
        LinkedList<T> treateadList = new LinkedList<>();
        for (T elemets : this) {
            if(predicate.test(elemets)){
                treateadList.add(elemets);
            }
        }
        return treateadList;
    }
    ...
    public T reduce(BinaryOperator<T> reducer, T accumulator) {
        for (T elements : this) {
            accumulator = reducer.apply(accumulator,elements);
        }
